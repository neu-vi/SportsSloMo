...
class SegAuxPipeline:
    def __init__(self,
        ...
        if resume:
            assert training, "Finetuning EBME with segmentation auxiliary loss"
            print("Restart optimizer state to fintune with segmentation loss.")
    ...
    def init_ppl_modules(self, load_pretrain=True):
        ...
        def load_pretrained_state_dict(module, module_name, module_args):
            ...
            return SegAuxPipeline.convert_state_dict(rand_state_dict, pretrained_state_dict)
    ...
    def print_grad(grad_input, grad_output):
        print("grad_input: ", grad_input)
        print("grad_output: ", grad_output)


    def train_one_iter(self, img0, img1, gt, seg, info, learning_rate=0,
            bi_flow_gt=None, time_period=0.5, loss_type="l2+0.1census+auxseg"):
        ...
        if loss_type == "l2":
        ...
        elif loss_type == "l2+0.1census+auxseg":
            loss_l2 = (((interp_img - gt) ** 2 + 1e-6) ** 0.5).mean()
            loss_ter = 0.1 * self.ter(interp_img, gt).mean()
            ...
            loss_G = loss_l2 + loss_ter + loss_panoptic
        ...
        return interp_img_return, bi_flow, loss_l2, loss_panoptic
