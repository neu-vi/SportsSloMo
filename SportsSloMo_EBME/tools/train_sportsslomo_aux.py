...
from core.sportsslomo_aux_ppl import AuxPipeline
from core.dataset import SportsSloMoDataset, SportsSloMoAuxDataset
...
def get_learning_rate(total_step, cur_step, init_lr, min_lr=1e-6):
    if cur_step < 2000:
        ...
        mul = np.cos((cur_step - 2000) / (total_step - 2000.) * math.pi) * 0.5 + 0.5
...
def train(ppl, dataset_cfg_dict, optimizer_cfg_dict):
    ...
    dataset_val = SportsSloMoDataset(dataset_name='validation', data_root=data_root)
    dataset_train = SportsSloMoAuxDataset(dataset_name='train', data_root=data_root)

    sampler = DistributedSampler(dataset_train)
    train_data = DataLoader(dataset_train, batch_size=batch_size,
            num_workers=nr_data_worker, pin_memory=True, drop_last=True, sampler=sampler)
    ...
    loss_type = optimizer_cfg_dict.get("loss_type", "l2+0.1census+auxseg")
    ...
    while step <  total_step+1:
        for i, data in enumerate(train_data):
            ...
            time_stamp = time.time()
            img_data, t_interp, seg, info_paths = data
            data_gpu = img_data.to(DEVICE, dtype=torch.float, non_blocking=True) / 255.
            info = [np.load(info_path, allow_pickle=True).tolist() for info_path in info_paths]
            t_interp_gpu = t_interp.to(DEVICE, dtype=torch.float, non_blocking=True)
            seg_gpu = seg.to(DEVICE, non_blocking=True)
            ...
            interp_img, bi_flow, loss_l2, loss_seg, loss_kpt = ppl.train_one_iter(
                    img0, img1, gt, seg_gpu, info, learning_rate=learning_rate, time_period = t_interp_gpu,
                    loss_type=loss_type)
            train_time_interval = time.time() - time_stamp
            time_stamp = time.time()

            if step % 100 == 1 and LOCAL_RANK == 0:
                writer.add_scalar('1-loss_l2', loss_l2, step)
                writer.add_scalar('2-learning_rate', learning_rate, step)
            if step % 1000 == 1 and LOCAL_RANK == 0:
                gt = (gt.permute(0, 2, 3, 1).detach().cpu().numpy() * 255).astype('uint8')
                interp_img = (interp_img.permute(0, 2, 3, 1).detach().cpu().numpy() * 255).astype('uint8')
                overlay = 0.5 * img0 + 0.5 * img1
                overlay = (overlay.permute(0, 2, 3, 1).detach().cpu().numpy() * 255).astype('uint8')
                bi_flow = bi_flow.permute(0, 2, 3, 1).detach().cpu().numpy()
                nr_show = min(6, batch_size)
                for i in range(nr_show):
                    imgs = np.concatenate((overlay[i], gt[i], interp_img[i]), 1)[:, :, ::-1]
                    writer.add_image(str(i) + '/0-overlay-gt-pred', imgs, step, dataformats='HWC')
                    writer.add_image(str(i) + '/1-flow_01_pred',
                            flow2rgb(bi_flow[i][:, :, :2]), step, dataformats='HWC')
                writer.flush()
            if LOCAL_RANK == 0:
                print('{} => train step: {}/{}; time: {:.2f}+{:.2f}; loss_l2: {:.4e}; loss_seg: {:.4e}; loss_kpt: {:.4e}'\
                        .format(EXP_NAME, step, total_step,\
                        data_time_interval, train_time_interval, loss_l2, loss_seg, loss_kpt))
            ...
            if (LOCAL_RANK == 0) and (step % save_interval == 0):
                ...
                logger.info('{} => val step: {}; psnr: {:.4f}'\
                        .format(EXP_NAME, step, psnr))
            step += 1

        dist.barrier()


def evaluate(ppl, step, val_data, writer_val):
    psnr_list = []
    ...
    if LOCAL_RANK == 0:
        ...
        logger.info('{} => val step: {}; psnr: {:.4f}'.format(EXP_NAME, step, psnr))
...
def init_exp_env():
    ...
    if (LOCAL_RANK == 0):
        ...
        if os.path.exists(THIS_EXP_LOG_DIR):
            while True:
                if prompt("Would you like to re-write the existing experimental saving dir?") == True:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
            description='train video frame interpolation model')

    #**********************************************************#
    # => args for basic information
    parser.add_argument('--exp_name', default="aux", type=str,
    ...
    parser.add_argument('--pyr_level', type=int, default=3,
            help='the number of pyramid levels')
    parser.add_argument('--flow_load_pretrain', type=bool, default=False,
            help='whether load pre-trained weight')
    parser.add_argument('--flow_model_file', type=str, default="",
            help='weight of the bi-directional flow model')
    parser.add_argument('--flow_fix_pretrain', type=bool, default=False,
            help='whether fix the pre-trained model weight')
    ...
    parser.add_argument('--fusion_load_pretrain', type=bool, default=False,
            help='whether load pre-trained weight')
    parser.add_argument('--fusion_model_file', type=str, default="",
            help='weight of the frame fusion model')
    parser.add_argument('--fusion_fix_pretrain', type=bool, default=False,
            help='whether fix the pre-trained model weight')
    ...
    parser.add_argument('--weight_decay', type=float, default=1e-4,
            help='wegith decay')
    parser.add_argument('--steps', type=float, default=1.4e6,
            help='total steps (iteration) for training')
    parser.add_argument('--save_interval', type=float, default=0.1e4,
            help='iteration interval to save model')
    parser.add_argument('--loss_type', type=str, default="l2+0.1census+aux",
            help='training loss')
    ...
    bi_flownet_args.load_pretrain = args.flow_load_pretrain
    bi_flownet_args.model_file = args.flow_model_file
    bi_flownet_args.fix_pretrain = args.flow_fix_pretrain

    fusionnet_args = argparse.Namespace()
    fusionnet_args.high_synthesis = args.high_synthesis
    fusionnet_args.load_pretrain = args.fusion_load_pretrain
    fusionnet_args.model_file = args.fusion_model_file
    fusionnet_args.fix_pretrain = args.flow_fix_pretrain
    ...
    Previous_EXP_LOG_DIR = "checkpoints"
    optimizer_cfg_dict["ckpt_file"] = os.path.join(THIS_EXP_LOG_DIR, "optimizer-ckpt.pth")
    if args.resume:
        module_cfg_dict["bi_flownet"].load_pretrain = True
        module_cfg_dict["bi_flownet"].model_file = os.path.join(
                Previous_EXP_LOG_DIR, "bi-flownet-EBME.pkl")
        module_cfg_dict["fusionnet"].load_pretrain = True
        module_cfg_dict["fusionnet"].model_file = os.path.join(
                Previous_EXP_LOG_DIR, "fusionnet-EBME.pkl")
    RESUME = args.resume
    ...
    ppl = AuxPipeline(module_cfg_dict, optimizer_cfg_dict, LOCAL_RANK,
            training=True, resume=RESUME)
