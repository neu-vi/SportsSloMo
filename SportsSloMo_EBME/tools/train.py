...
def train(ppl, dataset_cfg_dict, optimizer_cfg_dict):
    ...
    dataset_train = SportsSloMoDataset(dataset_name='train', data_root=data_root)
    ...
    loss_type = optimizer_cfg_dict.get("loss_type", "l2+0.1census")
    ...
    if RESUME:
        ...
        if step > total_step:
            raise ValueError("Previous trained steps have exceeded the total step of this experiment!")

    if LOCAL_RANK == 0:
        writer = SummaryWriter(tf_log_dir + '/train')
        writer_val = SummaryWriter(tf_log_dir + '/validate')
        if not RESUME:
            evaluate(ppl, step, val_data, writer_val)
    ...
    while step <  total_step+1:
        for i, data in enumerate(train_data):
            ...
            interp_img, bi_flow, loss_l2 = ppl.train_one_iter(
                    img0, img1, gt, learning_rate=learning_rate, time_period = t_interp_gpu,
                    loss_type=loss_type)
            ...
            if step % 1000 == 1 and LOCAL_RANK == 0:
                ...
                writer.flush()
            if LOCAL_RANK == 0:
                print('{} => train step: {}/{}; time: {:.2f}+{:.2f}; loss_l2: {:.4e}'\
                        .format(EXP_NAME, step, total_step,\
                        data_time_interval, train_time_interval, loss_l2))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', default="SportsSloMo", type=str,
            help='experiment name, will be used to save all generated files')
    parser.add_argument('--train_log_root', default="../SportsSloMo-train-log", type=str,
    ...
    parser.add_argument('--loss_type', type=str, default="l2+0.1census",
            help='training loss')
    ...
    ppl = Pipeline(module_cfg_dict, optimizer_cfg_dict, LOCAL_RANK,
